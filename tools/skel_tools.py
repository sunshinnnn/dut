...
def load_model(cfgs, useCuda=True, useDQ=None, device=None):
    if device is None:
    ...
    useDQ = useDQ if not useDQ is None else cfgs.useDQ
    ...
    def updateNode(self, dofParams, useDQ = None, returnT = False):
        ...
        if not returnT:
            if not useDQS_m:
                ...
                # skinRs = matrix_to_euler_angles(axis_angle_to_matrix(matrix_to_axis_angle(nodeVertTransformations[:,:3,:3]))).reshape(B,G,3)
                # for i in range(self.numVertG):
...
class SkinnedCharacter(nn.Module):
    ...
    def loadHandSeg(self):
        ...
        handIdxs = []
        if self.motion_base.shape[1] == 54:
        ...
        elif self.motion_base.shape[1] == 107:
            handJoints = ['LeftHand', 'LeftHandThumb1', 'LeftHandThumb2', 'LeftHandThumb3',
                             'LeftHandThumbEE', 'LeftHandIndex1', 'LeftHandIndex2', 'LeftHandIndex3',
                             'LeftHandIndexEE', 'LeftHandMiddle1', 'LeftHandMiddle2', 'LeftHandMiddle3',
                             'LeftHandMiddleEE', 'LeftHandRing1', 'LeftHandRing2', 'LeftHandRing3',
                             'LeftHandRingEE', 'LeftHandPinky1', 'LeftHandPinky2', 'LeftHandPinky3',
                             'LeftHandPinkyEE', 'LeftHandEE',
                             'RightHand', 'RightHandThumb1', 'RightHandThumb2', 'RightHandThumb3',
                             'RightHandThumbEE', 'RightHandIndex1', 'RightHandIndex2', 'RightHandIndex3',
                             'RightHandIndexEE', 'RightHandMiddle1', 'RightHandMiddle2', 'RightHandMiddle3',
                             'RightHandMiddleEE', 'RightHandRing1', 'RightHandRing2', 'RightHandRing3',
                             'RightHandRingEE', 'RightHandPinky1', 'RightHandPinky2', 'RightHandPinky3',
                             'RightHandPinkyEE', 'RightHandEE']
            for item in handJoints:
                handIdxs.append( self.skinBoneNames.index(item) )
        elif self.motion_base.shape[1] == 159:
            handJoints = ["left_wrist", "right_wrist",
        ...
        self.handMask = np.where(self.vertexLabels2Nunmpy>0)[0].tolist()
    ...
    def loadMesh(self):
        ...
        self.verts0, self.faces, self.uvs, self.facesuv = load_obj_mesh(self.meshPath, with_texture=True)
        ...
        self.textureMap = cv2.imread(self.meshPath.replace('obj','jpg'),-1)[:,:,::-1]
    ...
    def getSkelMeshHand(self, idx=0):
        """
            1. update/updateFrame
            2. getSkelMesh
        """
        return self.skeleton.getMeshHand(idx=idx)
    ...
    def update(self, dofParams : torch.Tensor):
        ...
        self.boneGlobalTransformations = self.skeleton.jointGlobalTransformations[:, self.skinBoneIdxs]




        self.jointTransformations = torch.einsum('bdmn, bdnk -> bdmk', self.boneGlobalTransformations, self.initialTransformations)


    def forward_cuda(self, dofParams, returnTransformation = False):
        self.skeleton.update_cuda(dofParams)
        self.B = self.skeleton.B

        # # self.boneGlobalTransformations = self.skeleton.jointGlobalTransformations[:, self.skinBoneIdxs]
        # self.jointTransformations = torch.einsum('bdmn, bdnk -> bdmk', self.skeleton.jointGlobalTransformations[:, self.skinBoneIdxs], self.initialTransformations)

        self.boneGlobalTransformations = self.skeleton.jointGlobalTransformations[:, self.skinBoneIdxs]
        self.jointTransformations = torch.einsum('bdmn, bdnk -> bdmk', self.boneGlobalTransformations, self.initialTransformations)
        self.vertTransformations = torch.einsum('bvd,bdmn->bvmn', self.skinWeight.repeat(self.B, 1, 1), self.jointTransformations)

        verts = torch.matmul(self.vertTransformations[:, :, :3, :3], self.verts0.reshape(1, self.numVert, 3, 1))[:, :,
                :, 0] + \
                self.vertTransformations[:, :, :3, 3]

        if returnTransformation:
            return verts, self.vertTransformations
        else:
            return verts

    def forward(self, dofParams, useDQ=None, returnTransformation = False):
        """
            Input:
                dofParams ==> B,D
...
class Skeleton():
    ...
    def loadSkeleton(self):
        ...
        for i in range(self.numDof):
            if 'scale' in self.dofNames[i] or 'updown' in self.dofNames[i]:
        ...
        self.pairsLabel = []
        self.handJointLabel = []
        for idx in range(6, len(self.jointNames)):
            if 'Hand' in self.jointNames[idx]:
                self.handJointLabel.append(True)
            else:
                self.handJointLabel.append(False)
            ...
            self.pairsMinus5.append( [idx - 5 , self.jointNames.index(self.jointParents[idx]) - 5] )
            # self.pairsMinus7.append( [idx - 7 , self.jointNames.index(self.jointParents[idx]) - 7] )
            if 'Hand' in self.jointNames[idx] and 'Hand' in self.jointParents[idx]:
                self.pairsLabel.append(True)
            else:
                self.pairsLabel.append(False)



        # self.pairsMinus6Body = []
    ...
    def compute_layered_parent_id(self, org_parent_id):
        ...
        occ_stat = np.ones(fa_stat.shape[0]).astype(np.bool_)
    ...
    def init_cuda(self):
        self.initCuda = True
        self.current_id_layered, self.parent_id_layered = self.compute_layered_parent_id(self.jointParentIdxs)
        ...
        print('init woot cuda skeleton')

    def update_cuda(self, dofParams):
        if not self.initCuda:
            self.init_cuda()
        ...
        # st1 = time.time()
        # print('step 0: {}'.format((st1 - st0) * 1000.0))

        self.jointParams = torch.mm(dofParams, self.dofJointWeights)
        # st2 = time.time()
        # print('step 1: {}'.format((st2 - st1) * 1000.0))
        ...
        jointLocalTransformation[:, self.revoluteList, :3, :3] = axis_angle_to_matrix( self.jointAxis[None][:,self.revoluteList]* self.jointParams[:, self.revoluteList][:,:,None])
        jointLocalTransformation[:, self.prismaticList, :3, 3] = self.jointAxis[None][:,self.prismaticList]* self.jointParams[:, self.prismaticList][:,:,None]
        jointLocalTransformation = self.jointLocalTranslation.repeat(B, 1, 1, 1).matmul( jointLocalTransformation  )
        # st3 = time.time()
        # print('step 2: {}'.format((st3 - st2) * 1000.0))
        # self.jointGlobalPositions
        self.jointGlobalTransformations = woot_cuda_skeleton.skeleton_fw(
            self.numJoint,
            self.current_id_layered,
            self.parent_id_layered,
            jointLocalTransformation
        )

        # st4 = time.time()
        # print('step 3: {}'.format((st4 - st3) * 1000.0))
        #
        # print('step full: {}'.format((st4 - st0) * 1000.0))
    ...
    def update(self, dofParams, updateMarker = False):
        ...
        #     else:
        #         if self.useDQ:
        #             parentGlobalTransformationDQ = torch.tensor([1, 0, 0, 0, 0, 0, 0, 0], dtype=torch.float32).repeat(B,
        #                                                                                                               1).to(
        #                 self.device)
        #         else:
        #             parentGlobalTransformation = torch.eye(4, dtype=torch.float32).repeat(B, 1, 1).to(self.device)
        ...
        #         self.jointGlobalTransformationsDQ[:, i] = imodDQTorch(parentGlobalTransformationDQ, dq_locals[:, i])
        #     else:
        #         self.jointGlobalTransformations[:, i] = parentGlobalTransformation.clone().matmul(
        #             jointLocalTransformation[:, i])
    ...
    def getMesh(self,idx=0):
        ...
        # mesh = create_skeleton(jointPositions[6:]/1000, pairs= self.pairsMinus6, r1=0.01, r2= 0.005)
        # mesh = create_skeleton(jointPositions[6:]/1000, pairs= self.pairsMinus6, r1=0.01, r2= 0.005)
    ...
    def getMeshHand(self,idx=0):
        jointPositions = to_cpu(self.jointGlobalPositions[idx]).reshape(-1, 3)
        # mesh = create_skeleton(jointPositions[6:]/1000, pairs= self.pairsMinus6[:])

        mesh = create_skeleton(jointPositions[6:] / 1000, pairs=self.pairsMinus6[:], handLabel=self.pairsLabel, handJointLabel=self.handJointLabel)
        mesh.vertices *= 1000.0
        return mesh
...
def loadMotion(motionPath, returnIdx= False, returnTensor=False, device = None ):
    ...
    motionIdxList = motionList[:, :1].astype('int').tolist()
...
def saveMotion(motionPath, motionList, motionIdxList = None):
    if not motionIdxList is None:
        ...
        if type(motionIdxList[0]) == int:
            motionIdxList = np.array(motionIdxList).astype('str')
    else:
