...
def listSub(list1, list2):
    ...
    return diff1
def read_depth(name):
    return cv2.imread(name, cv2.IMREAD_UNCHANGED).astype(np.float32) / 2.0 ** 14 #15

def save_depth(name, depth):
    cv2.imwrite(name, (depth * 2.0 ** 14).astype(np.uint16) )
...
def check_platform_dir(path,reverse=False):
    if not reverse:
        if platform == "linux" or platform == "linux2":
            if path[:2]=='Z:':
                path = '/HPS'+ path[2:]
...
def merge_images(images, rows, cols, interval):
    ...
    # Calculate the total size of the merged image
    total_width = (image_width + interval) * cols - interval
    total_height = (image_height + interval) * rows - interval

    # Create a new blank array for the merged image
    merged_image_array = np.zeros((total_height, total_width, 3), dtype=np.uint8)

    # Paste each image into the merged image array
    for i, img in enumerate(images):
        col_index = i % cols
        row_index = i // cols
        x_offset = col_index * (image_width + interval)
        y_offset = row_index * (image_height + interval)
        merged_image_array[y_offset:y_offset+image_height, x_offset:x_offset+image_width] = img
