...
def loadCSV(filename):
    # df = pd.read_csv(filename, delimiter=delimiter, usecols=usecols, nrows=nrows, header=header)
...
def saveCSV(filename, data):
    ...
    # format_str = "{:." + str(significant_digits) + "g}"
    format_str = "{:3.4f}"
    format_func = lambda x: format_str.format(x) if isinstance(x, float) else x
    df = df.applymap(format_func)

    df.to_csv(filename, index=False)
    print(f"save data successful to {filename}")
...
def depths_to_points(world_view_transform, full_proj_transform, H, W, depthmap):
...
def depth_to_normal(world_view_transform, full_proj_transform, H, W, depth):
    """
        view: view camera
        depth: depthmap
    """
    points = depths_to_points(world_view_transform, full_proj_transform, H, W, depth).reshape(*depth.shape[1:], 3)
