...
if __name__ == '__main__':
    ...
    base_dir = "/CT/HOIMOCAP4/work/data/DUT_official/{}/{}/{}".format(subjectType, clothType, sequenceType)
    ...
    if isSparse:
        out_dir = makePath(osp.join(base_dir, 'recon_neus2_sparse'))
    else:
    ...
    if subjectType == 'Subject0724':
        if sequenceType == 'training':
            indices = np.arange(100, 27000, 10).tolist()
            # indices = np.arange(100, 200, 10).tolist()
        elif sequenceType == 'testing':
            indices = np.arange(100, 8000, 10).tolist()
    elif subjectType == 'Subject0066':
        if sequenceType == 'training':
            indices = np.arange(100, 27500, 10).tolist()
        else:
            indices = np.arange(100, 8000, 10).tolist()
    elif subjectType == 'Subject0022':
        if sequenceType == 'training':
            indices = np.arange(110, 25000, 10).tolist()
        else:
            indices = np.arange(110, 7200, 10).tolist()
    elif subjectType == 'Subject0003':
        if sequenceType == 'training':
            indices = np.arange(100, 18000, 10).tolist()
        else:
            indices = np.arange(100, 7000, 10).tolist()

    if isSparse:
        indices = np.arange(1500, 2000, 1).tolist()
        # indices = np.arange(1500, 1505, 1).tolist()
        select_camera = True
        if subjectType == 'Subject0724':
            activeCamera = [47, 67, 77, 91]
        elif subjectType == 'Subject0066':
            activeCamera = [45, 65, 76, 88]
        elif subjectType == 'Subject0022':
            activeCamera = [47, 66, 77, 89]
        elif subjectType == 'Subject0003':
            activeCamera = [65, 77, 40, 57]
    ...
    if create_scene:
        ...
        if not isSparse:
            ...
            for idx, fIdx in enumerate(indices_csv):
                ...
                cameras = []
                for tIdx, camIdx in enumerate(activeCamera):
                    camera = {}
                    camera['pose'] = EXTR[camIdx]
                    camera['intrinsic'] = INTR[camIdx].copy()
                    camera['intrinsic'][0][2] -= centerOffsetAll[tIdx][idx][2]
                    camera['intrinsic'][1][2] -= centerOffsetAll[tIdx][idx][1]
                    cameras.append(camera)
                ...
                for tIdx, camIdx in enumerate(activeCamera):
                    ...
                    one_frame["transform_matrix"] = cameras[tIdx]['pose'].tolist()
                    ixt = cameras[tIdx]['intrinsic']
                    one_frame["intrinsic_matrix"] = ixt.tolist()
                    output['frames'].append(one_frame)
        ...
        if isSparse:
            p = Pool(numCpu)
            p.map(cropImgMask, activeCamera)
        else:
